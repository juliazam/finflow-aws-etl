...
| CloudWatch Logs does not record Glue Job Python Shell stdout | Diagnostics via writing debug data to S3 |
...
| Step Functions → Glue direct integration (`glue:startJobRun.sync`) is a stub and does not invoke actual Glue | Replaced with a Lambda intermediary using boto3 polling (`glue:StartJobRun`/`GetJobRun` in a loop) |
| AWS/Billing metrics (`EstimatedCharges`) are not published at all | No workaround — `billing_alarm` is kept in code as documentation of actual intent, not applied |